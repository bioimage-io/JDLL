...
public class RaiArrayUtils {
     ...
     *
     * @param rai  tensor where the copy is created from
     * @param type data type of the wanted tensor
     */
     ...
     * or the new tensor (including the name), except the data type of the data
     *
     * @param tt   tensor where the copy is created from
     * @param type data type of the wanted tensor
     */
    public static <T extends Type<T>> Img<T> createCopyOfRaiInWantedDataType(RandomAccessibleInterval<T> rai, Type type) {
        if (Util.getTypeFromInterval(rai) instanceof FloatType) {
            ...
            return createCopyOfUnsignedByteRaiInWantedDataType((RandomAccessibleInterval<UnsignedByteType>) rai, type);
        }
        else {
     ...
     *
     * @param rai  RandomAccessibleInterval where the copy is created from
     * @param type data type of the wanted tensor
     */
     ...
     * @param type data type of the wanted tensor
     */
    private static <T extends Type<T>> Img<T> createCopyOfUnsignedByteRaiInWantedDataType(RandomAccessibleInterval<UnsignedByteType> rai, Type type) {
        byte[] arr = RaiArrayUtils.unsignedbyteArray((RandomAccessibleInterval<UnsignedByteType>) rai);
        long[] tensorShape = rai.dimensionsAsLongArray();
        if (type instanceof DoubleType) {
            ArrayImg<DoubleType, DoubleArray> tensorBackend =
                ArrayImgs.doubles(RaiArrayUtils.convertByteArrIntoDoubleArr(arr), tensorShape);
    ...
    public static byte[] unsignedbyteArray(
        final RandomAccessibleInterval<UnsignedByteType> image) {
        final byte[] array = extractUnsignedByteArray(image);
        return array == null ? createUnsignedByteArray(image) : array;
    }
    ...
    private static byte[] createUnsignedByteArray(
        final RandomAccessibleInterval<UnsignedByteType> image) {
        final long[] dims = Intervals.dimensionsAsLongArray(image);
        final ArrayImg<UnsignedByteType, ByteArray> dest = ArrayImgs.unsignedBytes(dims);
        copy(image, dest);
        return dest.update(null).getCurrentStorageArray();
    }
    ...
    private static byte[] extractByteArray(
        ...
        @SuppressWarnings("unchecked") final ArrayImg<ByteType, ?> arrayImg = (ArrayImg<ByteType, ?>) image;
    ...
    private static byte[] extractUnsignedByteArray(
        final RandomAccessibleInterval<UnsignedByteType> image) {
        if (!(image instanceof ArrayImg)) return null;
        @SuppressWarnings("unchecked") final ArrayImg<UnsignedByteType, ?> arrayImg = (ArrayImg<UnsignedByteType, ?>) image;
        final Object dataAccess = arrayImg.update(null);
        return dataAccess instanceof ByteArray ? //
            ((ByteArray) dataAccess).getCurrentStorageArray() : null;
    }
    ...
    private static double[] extractDoubleArray(
        final RandomAccessibleInterval<DoubleType> image) {
        if (!(image instanceof ArrayImg)) return null;
        @SuppressWarnings("unchecked") final ArrayImg<DoubleType, ?> arrayImg = (ArrayImg<DoubleType, ?>) image;
        final Object dataAccess = arrayImg.update(null);
    ...
    private static float[] extractFloatArray(
        final RandomAccessibleInterval<FloatType> image) {
        if (!(image instanceof ArrayImg)) return null;
        @SuppressWarnings("unchecked") final ArrayImg<FloatType, ?> arrayImg = (ArrayImg<FloatType, ?>) image;
        final Object dataAccess = arrayImg.update(null);
    ...
    private static int[] extractIntArray(
        final RandomAccessibleInterval<IntType> image) {
        if (!(image instanceof ArrayImg)) return null;
        @SuppressWarnings("unchecked") final ArrayImg<IntType, ?> arrayImg = (ArrayImg<IntType, ?>) image;
        final Object dataAccess = arrayImg.update(null);
    ...
    private static long[] extractLongArray(
        final RandomAccessibleInterval<LongType> image) {
        if (!(image instanceof ArrayImg)) return null;
        @SuppressWarnings("unchecked") final ArrayImg<LongType, ?> arrayImg = (ArrayImg<LongType, ?>) image;
        final Object dataAccess = arrayImg.update(null);
