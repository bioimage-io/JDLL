...
public class ScaleLinearTransformation extends DefaultImageTransformation
{
	...
	private void checkCompulsoryArgs()
	{
		...
		{ throw new IllegalArgumentException(
				"Error defining the processing '" + name + "'. It should at least be provided with the "
						+ "arguments 'gain' and 'offset' in the" + " yaml file." ); }
	...
	{
		if ( axes == null && gainArr != null && gainArr.size() != 1 && offsetArr != null && offsetArr.size() != 1 )
		{ throw new IllegalArgumentException( "In order to specify the 'gain' and 'offset' parameters "
				+ "for several axes in the processing '" + name + "', please provide the 'axes' parameter too." ); } // else
																														// if
																														// (axes
																														// !=
																														// null
	 ...
	 * tensor.createCopyOfTensorInWantedDataType(DataType.FLOAT); INDArray arr =
	 * tensor.getData(); arr.mul(gain, arr); arr.add(offset, arr); return
	 * tensor; }
	 */
}
