...
public class ClipTransformation extends DefaultImageTransformation
{
	...
	private void checkCompulsoryArgs()
	{
		...
		{ throw new IllegalArgumentException(
				"Error defining the processing '" + name + "'. It should at least be provided with the "
						+ "arguments 'min' and 'max' in the" + " yaml file." ); }
	}
	...
	private void checkArgsCompatible()
	{
		if ( getFloatVal( min ) > getFloatVal( max ) )
			throw new IllegalArgumentException( "The argument 'min' for the processing routine " + "'" + name
					+ "' cannot be bigger than the other parameter 'max'." );
	}
