...
public final class DetectionMerger {
    ...
    private DetectionMerger() {}

    public static List<Detection> merge(final List<TileDetections> tileDetections) {
        return merge(tileDetections, DEFAULT_NMS_IOU_THRESHOLD);
    }
    ...
    public static List<Detection> merge(final List<TileDetections> tileDetections,
        ...
        for (TileDetections tile : tileDetections) {
            ...
            for (Detection detection : tile.getDetections()) {
                Detection global = toGlobalDetection(detection, tile.getWindow());
                if (global != null) {
                    shifted.add(global);
                }
            }
    ...
    private static Detection toGlobalDetection(final Detection detection, final long[] window) {
        if (detection == null || window == null || !isValid(detection)) {
            return null;
        }
        double x1 = clip(detection.getX1() + window[0], 0.0d, window.getImageWidth());
        double y1 = clip(detection.getY1() + window.getY(), 0.0d, window.getImageHeight());
        double x2 = clip(detection.getX2() + window.getX(), 0.0d, window.getImageWidth());
        double y2 = clip(detection.getY2() + window.getY(), 0.0d, window.getImageHeight());
        if (x2 <= x1 || y2 <= y1) {
            return null;
        }
        return new Detection(detection.getParentName(), window.getSampleIndex(), x1, y1, x2, y2,
    ...
    public static final class TileDetections {

        private final long[] window;
        private final List<Detection> detections;

        public TileDetections(final long[] window, final List<Detection> detections) {
            if (window == null) {
